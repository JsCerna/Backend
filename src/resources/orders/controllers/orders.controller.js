...
export const createOrder = async ( req, res ) => {
  ...
  const [ preferenceId, preferenceIdError ] = await awaitCatcher( mercadopago.preferences.create( { payer: { name: user.fullName, email: user.email }, back_urls:{success:'http://localhost:3000/profile'}, items }, ) )
